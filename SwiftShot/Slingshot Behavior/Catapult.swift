...
class Catapult: GameObject, Grabbable {
    ...
    public static func replaceCatapultPlaceholder(_ placeholder: SCNNode) -> SCNNode {
        ...
        node.name = "catapult"
        os_log(type: .info, "Catapult placeholder node %s replaced with %s", placeholder.name!, node.name!)
    ...
    init(_ node: SCNNode, sfxCoordinator: SFXCoordinator, identifier: Int, gamedefs: [String: Any]) {
        ...
        if abs(node.eulerAngles.x) > 0.001 || abs(node.eulerAngles.z) > 0.001 {
            os_log(type: .error, "Catapult can only have y rotation applied")
        }
    ...
    func serverGrab(cameraRay: Ray) {
        guard !isGrabbed else { os_log(type: .error, "Trying to grab catapult with player"); return }
        os_log(type: .debug, "(Server) Catapult%d grabbed by player", catapultID)
    ...
    func onGrabStart() {
        os_log(type: .debug, "(Player) Catapult%d grabbed by player", catapultID)
    ...
    func move(cameraInfo: CameraInfo) {
        ...
        guard isGrabbed else { os_log(type: .error, "trying to move before grabbing catapult"); return }
    ...
    func onLaunch(velocity: GameVelocity) {
        ...
        os_log(type: .debug, "Catapult%d launched", catapultID)
    ...
    func update() {
       ...
       if ballVisible == .hidden {
            ...
            if timeForCooldown < 0.01 {
                os_log(type: .error, "cooldown time needs to be long enough to play animations")
                timeForCooldown = 0.0
            }
